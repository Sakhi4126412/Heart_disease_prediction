...
if st.session_state.data_loaded:
    ...
    with tab_eda:
        ...
        if viz_type == "Distribution Analysis":
            ...
            with col1:
                feature = st.selectbox("Select Feature", data.select_dtypes(include=[np.number]).columns)
                fig = px.histogram(
                    data, x=feature, 
            ...
            if not numeric_data.empty:
                ...
                if 'HeartDisease' in corr_matrix.columns:
                    ...
                    correlations = corr_matrix['HeartDisease'].sort_values(ascending=False)
                    fig = px.bar(
            ...
            if 'HeartDisease' in data.columns and len(data.select_dtypes(include=['object']).columns) > 0:
                cat_col = st.selectbox("Select Categorical Feature", data.select_dtypes(include=['object']).columns)
        ...
        if st.button("🚀 Start Training", use_container_width=True):
            ...
            with st.spinner("Processing data and training models..."):
                ...
                try:
                    ...
                    st.warning("XGBoost not available. Skipping...")
                    models['XGBoost'] = None
                ...
                if results:
                    ...
                    if best_model in ['Random Forest', 'Gradient Boosting', 'Decision Tree', 'XGBoost']:
    ...
    with tab_prediction:
        ...
        if not st.session_state.get('models_trained', False):
            ...
            with col1:
                age = st.number_input("Age (years)", min_value=20, max_value=100, value=50, help="Patient's age")
                sex = st.selectbox("Gender", ['M', 'F'], help="Patient's gender")
            ...
            if st.button("🩺 Assess Heart Disease Risk", use_container_width=True):
                ...
                with col1:
                    if prediction == 1:
                        st.markdown("""
                        <div class="warning-message" style="animation: fadeIn 0.5s;">
                        ...
                        st.markdown("""
                        <div class="success-message" style="animation: fadeIn 0.5s;">
                ...
                if risk_factors:
                    risk_df = pd.DataFrame(risk_factors, columns=['Risk Factor', 'Details', 'Severity'])
                    for _, row in risk_df.iterrows():
                        color = "#dc3545" if row['Severity'] == "high" else "#ffc107"
                        st.markdown(f"""
                        <div style="border-left: 3px solid {color}; padding: 10px; margin: 5px 0; background: #f8f9fa;">
                            <strong>{row['Risk Factor']}</strong><br>
                            {row['Details']}
                        </div>
                        """, unsafe_allow_html=True)
                else:
...
<div style="text-align: center; color: #666; padding: 1rem;">
    <p>❤️ Heart Disease Prediction System | Powered by Machine Learning & AI</p>
